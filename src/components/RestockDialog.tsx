...
export function RestockDialog({ open, onOpenChange, lowStockItems }: RestockDialogProps) {
  ...
  const createRequestsMutation = useMutation({
      ...
      const requests = Object.entries(selectedItems)
        ...
        .map(([itemId, data]) => {
          ...
          return {
            ...
            reason: `Low stock alert - Current: ${item.quantity} ${item.unit}, Threshold: ${item.threshold} ${item.unit}`,
          };
        })
        .filter(Boolean);
    ...
    onSuccess: () => {
      toast.success("Restocking requests submitted successfully");
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Select items below threshold and specify quantities needed
          </p>
        </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-destructive mt-1">
                    {item.threshold - item.quantity} {item.unit} below threshold
                  </div>
                </div>
              </div>
